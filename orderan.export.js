// MEMENTO SCRIPT NAME: Export Web// TYPE: Action Script// TEMPAT ACTION: Bulk Actions Menu// REQUARED: moment.min.js
...
function ubahStatus(i) { e[i].set("Status Keep","Export");}

function bulatkan(n) { if (n>1000) { var h = (Math.floor((n/1000))*1000); var x = n - h; if (x<=200) { return (h/1000); } else { return (h/1000)+1; } } else { return 1 }}
...
var status = e[i].field("Status Keep");
...
var data = "{\"fields\":{\"barcode\":{\"stringValue\":\""+bcd+"\"},\"barang\":{\"stringValue\":\""+barang+"\"},\"warna\":{\"stringValue\":\""+warna+"\"},\"cs\":{\"stringValue\":\""+kodeCS+"\"},\"penerima\":{\"stringValue\":\""+penerima+"\"},\"toko\":{\"stringValue\":\""+toko+"\"},\"hargaBeli\":{\"integerValue\":\""+harga.toString()+"\"},\"status\":{\"stringValue\":\"export\"},\"pj\":{\"stringValue\":\"\"},}}"
//data = JSON.stringify(data).replace(/["]/g, '\\"');

if (status!="Export") { //verif

result = http.post(host, data);
var cek = result.body;
var error = cek.search(new RegExp("error","i"));
if (error==-1) { ubahStatus(i); jum++; }
else { message("FORMAT ERROR:\n"+data); }
if (e.length==jum) { message("Export Berhasil"); }
